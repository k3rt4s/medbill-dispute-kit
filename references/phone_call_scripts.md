# Phone-call scripts for medical-bill disputes
...
4. **Recording.** "Is this call being recorded? In that case I would like to record as well; do you have any objection?" In one-party-consent states (most), you may record without permission. In two-party-consent states (CA, CT, DE, FL, IL, MD, MA, MI, MT, NV, NH, PA, WA), get the rep's verbal consent on the recording. If they refuse, do not record; rely on contemporaneous notes.
...
You: "Thank you, [rep name]. I have three questions about this bill before I take any action. First: was this claim submitted to my health plan, [insurer name], member ID [member ID]? Second: do you have the corresponding Explanation of Benefits on file at the same balance? Third: can you send me an itemized statement with the CPT or HCPCS codes for every service line?"

[Listen. Note their answers verbatim.]
...
You: "Two follow-up questions: (a) was the qualifying payment amount used to calculate the allowed amount, and what was that QPA; and (b) if my provider was OON but the facility was IN-network, did you treat this as an OON ancillary case under § 300gg-111(b)?"
...
You: "Hello, this is [name], DOB [DOB], account [account number]. I received a notice dated [date] stating this account will be referred to collections on [referral date]. The account is in active written dispute. May I have your name and employee ID?"

[After introduction.]

You: "This is an account in active dispute. I sent a written dispute letter on [date] by certified mail, USPS tracking [tracking number]. Under [the FDCPA, if a third-party collector is involved, or under your own published billing-dispute policy], collection activity must be held while the dispute is resolved. I am calling to:
(a) confirm that this account will not be referred to collections on [referral date];
(b) confirm that no negative information will be reported to consumer credit bureaus during the dispute;
(c) get the rep's name, ID, and call-reference number; and
(d) confirm a written acknowledgment will be mailed or emailed today."
...
You: "Hello, this is [name], DOB [DOB], account [account number]. I would like to speak with the patient advocate or financial counselor for [hospital name]. May I have that person's name and direct contact information?"

[If they say there is no such role: "Then I would like to speak with the supervisor of the patient-financial-services department, please. May I have that person's name?"]
...
You: "Hello, my name is [name]. I am a patient with an active billing dispute against [hospital name] that I have been unable to resolve through patient financial services or the patient advocate. May I leave a brief message for [CFO name]?"

[Voicemail script:]

"[CFO name], my name is [name], date of birth [DOB], account number [account number]. I have an unresolved billing dispute that has gone through your patient-financial-services department and your patient advocate without a written response addressing the substance. The dispute is summarized in my letter dated [date], certified mail tracking [tracking number], which I am also emailing to your office today.

Before I escalate to the [state DOI / state Attorney General / civil court], I am giving the hospital a final opportunity at the executive level to respond in writing within ten business days. My phone is [phone number]. Thank you."
...
[Get all of it. End the call without confirming the debt is yours, without authorizing payment, and without scheduling a callback. Send the written FDCPA validation letter the same day via `templates/letter_fdcpa_validation.md`.]

[Note: under § 1692g, you have 30 days from the collector's first communication to dispute in writing; the collector must cease collection activity until validation is provided. The phone call alone does not invoke § 1692g(b); the written request does. The phone call is to get the collector's information so you can mail the letter.]

---

## What not to say on a phone call

- "Yes, that's my debt." (Confirms a debt that may not be enforceable as billed; do not concede.)
- "I'll pay $X over Y months." (Verbal commitment may be treated as a re-aged debt under FCRA, restarting the SOL clock.)
- "I just want this over." (The rep's job is to extract; do not signal a deadline pressure.)
- "I don't have the paperwork in front of me." (Always have it; if you don't, end the call and reschedule.)
- "Let me just give you my card number." (Never pay on a verbal call; written settlements only.)

## State-specific recording rules

- **One-party consent** (you alone can record): AL, AK, AZ, AR, CO, DC, GA, HI (subject to RS 711-1111), ID, IN, IA, KS, KY, LA, ME, MN, MS, MO, NE, NV (federal), NJ, NM, NY, NC, ND, OH, OK, RI, SC, SD, TN, TX, UT, VT, VA, WV, WI, WY.

- **All-party consent** (everyone on the call must consent): CA, CT, DE, FL, IL (recent narrowing), MD, MA, MI (recent narrowing), MT, NH, OR (with exceptions), PA, WA.

When in doubt, ask. The recording is a tool, not the case; a clear contemporaneous note set is almost as defensible if challenged.

---

## How to use this file with the rest of the kit

1. Log every meaningful call in `actions.csv` via `scripts/log_interaction.py --action phone_call_to_billing` (or `_to_insurance` / `_to_billing_advocate` / `_to_cfo`).
...
4. If the call yielded a settlement offer, do not accept verbally. Request the offer in writing, signed by an authorized representative, before sending payment.

---

## A note on the kit's mail-only default

The state machine in `scripts/draft_letters_by_state.py` does not draft phone scripts and does not assume calls will occur. If you make calls, the action log is the only place that record lives in the kit. The dispute drafter pulls the action log into its prompt context when drafting later letters, so a well-logged call ("rep promised hold on collections through 2026-06-30") flows into the next letter automatically.

If you are coming to this kit from Marshall Allen's book and expecting phone-first, you will find the kit is letter-first. The phone scripts above are for the calls you do choose to make; the kit will not generate them automatically and will not depend on them succeeding.
