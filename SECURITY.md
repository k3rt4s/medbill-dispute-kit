...
The optional local-operations pipeline in `scripts/` is executable Python and is not required to use the instruction kit. Static inspection of all 20 Python files, reviewed again on 2026-09-09 after the Form 990, SBC, and dispute-reply-classifier additions, found:
...
- `parse_990.py` and `parse_sbc.py` import PyMuPDF (as `pymupdf`) and the OpenAI client. Unlike the two scripts above, they extract the PDF's text layer locally and send that extracted text, not images, to the configured Azure OpenAI endpoint, and only as a fallback: `parse_990.py` calls the model only for `--pdf` input, never for `--xml`, which stays fully offline; `parse_sbc.py` calls the model only for fields its deterministic regex extraction left null. `parse_990.py` also accepts an optional `--fetch-from` URL to download its own input file over standard-library HTTP, a separate action from the model call.
