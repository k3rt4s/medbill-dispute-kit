...
- File a parallel state-DOI complaint via `templates/complaint_state_doi.md`, Category B (NSA balance-billing).
- If the provider continues to send statements while this letter is pending, do not pay; the federal rule fixes the patient's cost-sharing.
...
## When IDR is not the patient's right
...
- For self-pay / uninsured patients, the analogous process is Patient-Provider Dispute Resolution (`templates/letter_ppdr_initiate.md`), not IDR.
